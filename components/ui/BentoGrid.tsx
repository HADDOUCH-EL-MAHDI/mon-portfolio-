...
export const BentoGridItem = ({
  ...
  const leftLists = ["ReactJS", "Angular", "Mui"];
  const rightLists = ["Spring boot", "Git", "Docker", "Scrum", "UML", "SQL"];
  ...
  const handleCopy = () => {
    const text = "haddouchelmahdi@gmail.com";
          ...
          {id === 3 && (
            <div className="flex flex-wrap gap-2 lg:gap-4 mt-4">
              {leftLists.concat(rightLists).map((item, i) => (
                <span
                  key={i}
                  className="py-2 px-3 text-xs lg:text-sm opacity-80 rounded-lg text-center bg-[#10132E] text-white"
                >
                  {item}
                </span>
              ))}
